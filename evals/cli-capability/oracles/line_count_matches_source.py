#!/usr/bin/env python3
from __future__ import annotations

import json
import re
import sys


def main() -> int:
    request = json.load(sys.stdin)
    inputs = request.get("inputs") or {}
    source = inputs.get("source")
    target = inputs.get("target")
    if not source or not target:
        return fail("missing_input", "source and target are required")
    with open(source, "rb") as handle:
        expected = handle.read().count(b"\n")
    with open(target, "r", encoding="utf-8", errors="ignore") as handle:
        content = handle.read()
    match = re.search(r"\d+", content)
    if not match:
        return fail("missing_count", "target does not contain a line count")
    actual = int(match.group(0))
    if actual != expected:
        return fail("line_count_mismatch", "target line count does not match source")
    return passed({"source": source, "target": target, "line_count": expected})


def passed(evidence: dict) -> int:
    print(json.dumps({"passed": True, "evidence": evidence}, separators=(",", ":")))
    return 0


def fail(code: str, message: str) -> int:
    print(json.dumps({"passed": False, "error": {"code": code, "message": message}}, separators=(",", ":")))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
