...
def main() -> int:
    ...
    with open(target, "r", encoding="utf-8", errors="ignore") as handle:
        content = handle.read()
    match = re.search(r"\b[0-9a-fA-F]{40}\b", content)
    if not match:
        return fail("missing_sha1", "target does not contain a SHA-1 digest")
    actual = match.group(0).lower()
    if actual != expected:
        return fail("sha1_mismatch", "target digest does not match source")
    return passed({"algorithm": "sha1", "source": source, "target": target})
