# CAL CLI v0 Benchmark

This directory reserves the first stable CAL seed benchmark surface for release
evaluation.

The benchmark is intentionally not the same as the current experiments:

- experiments ask whether a capability-acquisition path can produce useful
  evidence;
- benchmarks fix tasks, providers, scoring, and baselines so results can be
...
```text
evals/benchmarks/cal-cli-v0/
  README.md
  tasks.jsonl      # fixed task families and held-out reuse fixtures
...
The benchmark must not trust those verifier packages as final task evidence.
Held-out reuse outputs are scored by fixed scripts under `oracles/`.
...
The current checked-in task schema is a bootstrap slice with five task families.
...
- held-out task success: intent-level Use selection, runtime execution on reuse
  fixtures, and benchmark oracle scoring;
...
- CAL replay/live: the CAL acquisition loop with deterministic verification,
  promotion, later intent-level Use, and runtime reuse.
...
under `evals/out/benchmarks/cal-cli-v0/`.

For reported release results, commit only sanitized release artifacts or trace
excerpts that correspond to the reported tables. Keep raw local run outputs, API
keys, raw secret-bearing prompts, and machine-specific dumps out of git.
