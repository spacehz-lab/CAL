...
func runAcquisitionForProviderPath(t *testing.T, repo string, env []string, calctlBin string, providerPath string, args ...string) (model.Provider, contract.AcquisitionResponse) {
	t.Helper()
	provider := addProvider(t, repo, env, calctlBin, providerPath)
	var response contract.AcquisitionResponse
	command := append([]string{"acquisition", "run", "--provider-id", provider.ID, "--stream"}, args...)
	events := runJSONStream(t, repo, env, &response, calctlBin, command...)
	if response.Error != nil || response.CapabilitiesPromoted == 0 {
		t.Logf("acquisition stream events:\n%s", formatStreamEvents(events))
	}
