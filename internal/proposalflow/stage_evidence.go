...
func (proposer *LLMProposer) planEvidence(ctx context.Context, req Request, candidateIndex int, candidate caltrace.Candidate, material probeMaterial) (evidenceStageOutput, []byte, error) {
	content, err := proposer.client.Complete(ctx, cliEvidencePrompt(req, candidateIndex, candidate, material))
	if err != nil {
		return evidenceStageOutput{}, nil, fmt.Errorf("evidence stage: %w", err)
	}
	var output evidenceStageOutput
	if err := json.Unmarshal(content, &output); err != nil {
		return evidenceStageOutput{}, content, fmt.Errorf("decode evidence stage: %w", err)
	}
	if !core.ValidVerifierID(output.Verifier.ID) {
		return evidenceStageOutput{}, content, fmt.Errorf("evidence stage returned invalid verifier id %q", output.Verifier.ID)
	}
	packageIDs := map[string]struct{}{}
	for index, pkg := range output.VerifierPackages {
		if !core.ValidVerifierID(pkg.ID) {
			return evidenceStageOutput{}, content, fmt.Errorf("evidence verifier package %d id %q is invalid", index, pkg.ID)
		}
		if strings.HasPrefix(pkg.ID, "verifier_") {
			return evidenceStageOutput{}, content, fmt.Errorf("evidence verifier package id %q must not start with verifier_", pkg.ID)
		}
		if strings.TrimSpace(pkg.VerifyPY) == "" {
			return evidenceStageOutput{}, content, fmt.Errorf("evidence verifier package %q verify_py is required", pkg.ID)
		}
		packageIDs[pkg.ID] = struct{}{}
	}
	if len(output.VerifierPackages) > 0 {
		if _, ok := packageIDs[output.Verifier.ID]; !ok {
			return evidenceStageOutput{}, content, fmt.Errorf("evidence verifier %q has no matching verifier package", output.Verifier.ID)
		}
	}
	return output, content, nil
}

func finalVerifier(output evidenceStageOutput, hash string, capabilityIndex int, candidateIndex int) (core.Verifier, error) {
