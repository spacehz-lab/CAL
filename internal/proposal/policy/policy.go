...
// SurfacePolicy controls surface material accepted for capability planning.
	...
	return Policy{
		...
		Capability: CapabilityPolicy{
			PreferredSubjects: []string{
				"file", "directory", "path", "text", "bytes", "json", "xml", "csv", "yaml",
				"document", "pdf", "image", "audio", "video", "archive", "package",
				"source", "repository", "database", "table", "http", "url", "network",
				"process", "system", "environment", "key", "certificate", "container", "project",
			},
			PreferredOperations: []string{
				"read", "write", "copy", "move", "remove", "list", "find", "inspect",
				"identify", "count", "search", "filter", "query", "extract", "transform",
				"convert", "render", "encode", "decode", "compress", "decompress",
				"create", "checksum", "sign", "verify", "encrypt", "decrypt", "download",
				"upload", "request", "sync", "install", "update", "build", "test",
				"run", "format", "validate", "sort", "deduplicate", "compare",
			},
		},
	}
}

// Validate checks that a policy can be applied locally.
func Validate(policy Policy) error {
	if err := ValidateSurface(policy.Surface); err != nil {
		return err
	}
	return ValidateCapability(policy.Capability)
}

// ValidateSurface checks surface policy fields.
func ValidateSurface(policy SurfacePolicy) error {
...
// ValidateCapability checks capability policy fields.
func ValidateCapability(policy CapabilityPolicy) error {
	...
	return validateCapabilityTerms("operation", policy.PreferredOperations)
}

// NormalizeToken normalizes policy vocabulary tokens.
