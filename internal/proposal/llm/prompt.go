...
existing_capability_ids is a bounded local lookup result, not the full catalog. Prefer one existing_capability_ids value when it semantically matches the observed operation. Generate a new capability_id when no existing_capability_ids value matches.

Capability ids must be provider-independent semantic operation ids, not provider names, executable names, command names, menu labels, flags, paths, versions, marketing labels, temporary task names, or random suffixes. New capability ids must use lowercase dotted words matching <domain>.<verb_object>, such as archive.create_zip or file.hash_sha1. If the request hint is present and matches the observed capability, use it as capability_id.

Do not create broad capability ids. If execution.spec.args hard-codes an output type, result type, format, encoding, archive type, checksum algorithm, or target artifact kind, include that fixed result discriminator in capability_id. Examples include, but are not limited to, document.export_pdf, document.convert_html, plist.convert_json, image.convert_png, archive.create_zip, file.compress_gzip, text.encode_base64, file.hash_sha1, or file.hash_md5.

If observations expose a result discriminator as a runtime input, such as format, encoding, archive type, checksum algorithm, or output artifact kind, prefer one parameterized capability instead of many fixed-value capabilities. The capability_id for a parameterized operation must name the runtime discriminator in the operation, such as document.convert_format or file.hash_algorithm; do not use an overly broad id such as document.convert when the result is selected by format. A parameterized candidate must use the same runtime placeholder in execution.spec.args, describe the placeholder in input_constraints, record documented accepted values or a verified subset in input_constraints.enum when observations provide them, say in description that the result is selected by runtime input, and use a verifier that reads the same runtime input and validates the matching outcome. If observations support a parameterized operation but only some documented values can be meaningfully verified, emit the parameterized candidate with input_constraints.enum limited to that verified documented subset instead of returning an empty candidates array.
