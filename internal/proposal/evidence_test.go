...
func TestDraftEvidenceDropsFixtureOnlyLiteralCheck(t *testing.T) {
	client := &fakeStageClient{responses: [][]byte{[]byte(`{"verify":{"level":"L3","method":"execute","checks":[{"subject":{"type":"file","input":"target"},"predicate":"exists","params":{}},{"subject":{"type":"file","input":"target"},"predicate":"contains","params":{"value":"hello"}},{"subject":{"type":"exit_code"},"predicate":"equals","params":{"value":0}}]}}`)}}
...
func TestDraftEvidenceKeepsObservationBackedLiteralCheck(t *testing.T) {
	client := &fakeStageClient{responses: [][]byte{[]byte(`{"verify":{"level":"L3","method":"execute","checks":[{"subject":{"type":"file","input":"target"},"predicate":"contains","params":{"value":"CAL_PROBE_OK"}},{"subject":{"type":"exit_code"},"predicate":"equals","params":{"value":0}}]}}`)}}
	output, _, err := NewLLMProposer(client).draftEvidence(context.Background(), Request{
		Provider: core.Provider{ID: "provider_cli", Kind: core.ProviderKindCLI},
...
func TestDraftEvidenceFiltersFixtureOnlyContainsAnyValues(t *testing.T) {
	client := &fakeStageClient{responses: [][]byte{[]byte(`{"verify":{"level":"L3","method":"execute","checks":[{"subject":{"type":"file","input":"target"},"predicate":"contains_any","params":{"values":["hello","CAL_PROBE_OK"]}}]}}`)}}
	req := Request{
		Provider: core.Provider{ID: "provider_cli", Kind: core.ProviderKindCLI},
		Observations: []caltrace.Observation{{
			Type:    "cli_output",
